...
<div class="table-responsive">
    <div class="d-none" id="popup-action">
        <div class="show">
            <a href="javascript:void(0)" class="btn btn-danger mb-3 text-light mt-2 ml-2" onclick="shop.admin.checkAllAction('feature', 'status', -1)" title="Xóa tất cả bản ghi đã chọn">Xóa</a>
        </div>
    </div>
    <table id="table-extended-chechbox" class="table table-transparent">
        ...
        @foreach($lsObj as $k=>$item)
            <tr>
                ...
                <td>
                    <h5><span class=""><i class="mdi mdi-clock-in"></i> {{ \Lib::dateFormat($item['created'], 'd/m/Y - H:i') }}</span></h5>
                </td>
                {{-- <td>
                    @php($positions = $item->positions())
                    @if($positions)
                        <span class="mb-2 sp-line-2">
                            @foreach($positions as $v)
                                <span class="badge badge-primary badge-pill">{{ (!$loop->first?' ':'').$v['title'] }}</span>
                            @endforeach
                        </span>
                    @endif
                </td> --}}
                <td align="center">
                    <img width="250px" src="{{ $item->getImageUrl('medium', 'image') }}" alt="table-user" class="mr-2"/>
                </td>
                <td>
                    @if($item->status != -1)
