    ...
    <div class="card">
            ...
            <h5 class="card-title">
                ...
                <span>
                    <a href="{{ route('admin.'.$key.'.add.post') }}"
                        class="btn btn-info fix-add-btn" data-overlaycolor="#38414a">
                        Thêm mới
                    </a>
                </span>
            </h5>
            ...
            <form id="search">
                <div class="heading-elements">
                    <ul class="list-inline mb-0">
                        <li>
                            <fieldset class="has-icon-left" data-toggle="tooltip" data-placement="top"
                                title="Tìm kiếm theo trạng thái">
                                <select name="status" class="form-control" data-toggle="select2">
                                    <option value="">Chọn trạng thái </option>
                                    <option value="1"
                                        {{ $search_data->status == '1' ? ' selected="selected"' : '' }}>
                                        Đã đăng</option>
                                    <option value="0"
                                        {{ $search_data->status == '0' ? ' selected="selected"' : '' }}>
                        ...
                        <li>
                            <fieldset class="has-icon-left" data-toggle="tooltip" data-placement="top"
                                title="Tìm kiếm theo ngày khởi tạo">
                                <input style="width: 200px;" autocomplete="off" type="text" name="time_between"
                                    value="{{ $search_data->time_between }}" class="form-control range-datepicker"
        ...
        <div class="table-responsive">
            <div class="d-none" id="popup-action">
                <div class="show">
                    <a href="javascript:void(0)" class="btn btn-danger mb-3 text-light mt-2 ml-2" onclick="shop.admin.checkAllAction('icon', 'status', -1)" title="Xóa tất cả bản ghi đã chọn">Xóa</a>
                </div>
            </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('admin.'.$key.'.edit', $item->id) }}"
                                    class="sp-line-1 ext-bold-600 pl-0">{{ $item->title }}</a>
                            </td>

                            <td align="center">
                                <img width="150px" height="100"
                                    src="{{ $item->getImageUrl('medium', 'image') }}"
                                    alt="table-user" class="mr-2" />
                            </td>
                            ...
                            <td align="center">
                                @if($item->status != -1)
                                    <div class="switchery-demo" title="Trạng thái hiển thị">
                                        <input type="checkbox" class="switchery" data-plugin="switchery"
                                            data-id="{{ @$item->id }}" data-color="#00695c" @if($item->status)
                                        checked @endif data-size="small"/>
                                    </div>
                                @else
                            ...
                            <td align="center">
                                <h5><span class="" style="font-size: 1rem"><i class="mdi mdi-clock-in"></i>
                                        {{ \Lib::dateFormat($item['created'], 'd/m/Y - H:i') }}</span>
                                </h5>
                            </td>
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        @if(\Lib::can($permission, 'edit'))
                                            <a title="Cập nhật thông tin"
                                                href="{{ route('admin.'.$key.'.edit', $item->id) }}"
                                        ...
                                        @if(\Lib::can($permission, 'delete'))
                                            <a title="Xóa bản ghi"
                                                href="{{ route('admin.'.$key.'.delete', $item->id) }}"
