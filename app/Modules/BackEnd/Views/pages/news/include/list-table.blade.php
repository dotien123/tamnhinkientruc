    ...
    <div class="card">
        ...
        <div class="table-responsive">
            <div class="d-none" id="popup-action">
                <div class="show">
                    <a href="javascript:void(0)" class="btn btn-danger mb-3 text-light mt-2 ml-2" onclick="shop.admin.checkAllAction('news', 'status', -1)" title="Xóa tất cả bản ghi đã chọn">Xóa</a>
                </div>
            </div>
            <table id="table-extended-chechbox" class="table table-transparent">
                ...
                <tr>
                    ...
                    <td class="text-bold-600 pl-0" style="width: 350px">
                        {{--<img src="{{ $value->getImageUrl() }}" alt="{{ @$value->title }}" class="mr-1" height="36" width="27">--}}
                        {{ $value->title }}
                    </td>
                    {{-- <td style="width: 100px">
                        <span class="text-success">
                            @foreach ($value->cate as $z => $item)
                            @if($z>0)
                                ,
                            @endif
                            {{$item->title}} 
                    ...
                    <td style="width:120px">
                        @if($value->status == \App\Models\News::STATUS_ACTIVE)
                            <span class="text-success">Đã đăng</span>
                        @elseif($value->status == \App\Models\News::STATUS_INACTIVE)
                            <span class="text-warning">Chờ xét duyệt</span>
                        @elseif($value->status == \App\Models\News::STATUS_DRAFF)
