            ...
            <div class="fs-14 fc-black" align="center">
                @if ($err != '')
                    ...
                    <div class="popup-form-btn">
                        <button class="fs-16 fw-rbb" onclick="shop.sendActiveMail({{$customer->id}})">Gửi lại email kích hoạt</button>
                    </div>
                    @endif
                @else
                    <p class="fw-ob">Chúc mừng bạn đã kích hoạt tài khoản thành công.</p>
                    <p>Bạn đã có thể sử dụng email <span class="fw-ob">{{ $customer->email }}</span> để <a href="#" data-toggle="modal" data-target="#loginModal" class="fw-ob read-more">Đăng nhập</a> vào hệ thống ngay bây giờ.</p>
