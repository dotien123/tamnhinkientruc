...
class Customer extends Authenticatable {
    ...
    public static function createOne($request) {
        ...
        $customer->user_name = !empty($request['name']) ? strtolower($request['name']) : $fullname;
        ...
        $customer->active = !empty($request['active']) ? $request['active'] : 1;
        $customer->status = 1;
...
//            CustomerToken::where('phone' , '=',  $customer->phone)->delete();
//            //tao token de active
...
//            $customerToken->save();
            ...
            if (empty($subscriber)) {
                ...
                $subscriber->fullname = $customer->fullname;
    ...
    public static function changeProfile($user , $result){
        if (!empty($result['fullname'])) {
            $fullname = $result['fullname'];
        } else {
            $fullname = explode('@', $result['email']);
            $fullname = $fullname[0];
        }

        //email truoc khi save
        $user_email_old = $user->email;

        if(!empty($user)){
            $user->fullname = $fullname;
            $user->email = $result['email'];
            $user->phone = !empty($result['phone']) ? $result['phone'] : '';
            $user->reg_ip = request()->ip();
            $user->created = time();
            $user->save();

            if (!empty($user_email_old)) {
                CustomerToken::whereEmail($user_email_old)->delete();
                //tao token de active
                $customerToken = new CustomerToken();
                $customerToken->email = $user->email;
                $customerToken->created_at = time();
                $customerToken->customer_id = $user->id;
                $customerToken->type = 1;
                $customerToken->token = $customerToken->token();
                $customerToken->save();

                //them vao subscribe
                $subscriber = Subscriber::where('email', $user_email_old)->first();
                if (!empty($subscriber)) {
                    $subscriber->customer_id = $user->id;
                    $subscriber->email = $user->email;
                    $subscriber->fullname = $user->fullname;
                    $subscriber->phone = $user->phone;
                    $subscriber->created = time();
                    $subscriber->save();
                }
            }
