...
class Lib{
    ...
    public static function robotSEO($id = false) {
        $robots = [
            '4' => ['title' => 'INDEX, FOLLOW', 'id' => '4'],
            '3' => ['title' => 'INDEX, NOFOLLOW', 'id' => '3'],
            '2' => ['title' => 'NOINDEX, FOLLOW', 'id' => '2'],
            '1' => ['title' => 'NOINDEX, NOFOLLOW', 'id' => '1'],
        ];
        if($id && isset($robots[$id])) {
            return $robots[$id]['title'];
        }
        return $robots;
    ...
    public static function siteTitle($title = '', $def = '', $is_admin = false){
        ...
        if($is_admin) {
            return '<title>' . ($title == '' ? $host : ($title . ' - CMS.YP-Love')) . '</title>';
        }
        return '<title>' . ($title == '' ? $host : ($title)) . '</title>';
    }
    ...
    public static function numberFormat($number = 0, $sysfor = '.'){
        if($number >= 1000){
            return number_format($number,0,',',$sysfor);
        }
        return $number;
    }
...
//        $str = preg_replace('/[^\u4E00-\u9FFF]+/', '', $str);
    ...
    public static function priceFormat($price = 0, $del = ',', $currency = ',00 VNĐ'){
        if($currency === ''){
            $currency = self::getSiteConfig('currency', 'VNĐ');
        }
        return self::numberFormat($price, $del)."$currency";
    }
    ...
    public static function dateFormat($time = 0, $format = 'd/m/Y', $vietnam = false, $show_time = false){
        if(!is_int($time)){
    ...
    public static function dateFormat1($time = 0, $format = 'H:i', $vietnam = false, $show_time = false){
        if(!is_int($time)){
            $time = date_create($time)->getTimestamp();
        }
        $return = date($format,$time);
        if ($vietnam){
            $return = ($show_time ? date('H:i - ',$time) : '') . self::$days[date('D',$time)] . ', ngày ' . date('H:i',$time);
        }
        return $return;
    }
    ...
    public static function duration($youtube_time) {
    ...
    public static function is_profanity($str, $replace = false)
    {
        $str = strtolower($str);
        $filtradas = 'ding, shit, địt mẹ, địt, má mày, móe, fuck, fucking, thằng, thằng bố mày, bố mày, mày, thằng cụ mày, thằng chó, tiên sư, tiên sư mày, tổ cha, tổ cha mày, vãi, vãi cứt';
        $f = explode(',', $filtradas);
        $f = array_map('trim', $f);
        $filtro = implode('|', $f);
        return ($replace) ? preg_replace("#$filtro#i", $replace, $str) : preg_match("#$filtro#i", $str) ;
    }
    ...
    static function convertMoneyToInt($string, $del = ',', $cur = 'VNĐ') {
        if(is_string($string)) {
            $string = explode($del,$string)[0];
        }
        return intval(str_replace(['.' , ','], '', $string));
    }

    static function convertIntToMoney($stringNumber, $sep = '.') {
        if(!$stringNumber){
            return $stringNumber;
        }
    ...
    public static function replace_key_by_val($arr = [],$key_str = '') {
        if($arr){
            $temp = [];
            foreach($arr as $k => $v) {
                if(is_array($v)){
