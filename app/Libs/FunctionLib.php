...
class FunctionLib{
    ...
    public static function appEnv(){
        if(empty(self::$env)){
            ...
            if($app_env != 'admin') {
                ...
                if(\Lib::getSiteConfig('mobile_active', 0) == 1 && $mobile) {
                    $app_env = 'mobile';
                }
            }
            self::$env = $app_env;
    ...
    public static function price_format($price,$unit = 'đ'){
        return $price >= 0 ? number_format($price) . ' ' . $unit : '';
    }
    ...
    static function checkActiveMenu($routes = ''){
        $current = \Route::currentRouteName();
        $routes = $routes.';';
        $position = stripos($routes, $current.';');
        if($position!==false) {
            return $position == 0 || $routes[$position - 1] == ';';
        }
        return false;
    }
    ...
    public static function tplShareGlobal($module_dir = ''){
        ...
        $config2 = [
            ...
            'site_js_val' => [
                ...
                'CURRENCY' => self::config('currency', 'đ'),
    ...
    public static function renderBreadcrumb($extraCommand = [], $noExtra = false,$blade = ''){
        $html = '';
        if(!empty(self::$breadcrumb)) {
            $defVal = self::defaultBreadcrumb();
            if($defVal['folder'] == 'BackEnd' && !$noExtra && empty($extraCommand)){
                $routeName = \Route::current()->getName();
                $adminIgnoreKey = ['admin.config', 'admin.home', 'home', 'admin.gallery'];
                if(!in_array($routeName, $adminIgnoreKey)) {
            ...
            $html = \View::make( ($blade ? $blade : $defVal['folder'].'::layouts.breadcrumb'), [
    ...
    public static function siteTitle($title = '', $def = '', $is_admin = false, $nohost = false){
        $host = \Request::getHost();
        $title = trim($title != '' ? $title : $def);
        if($is_admin) {
            return '<title>' . ($title == '' ? $host : ($title . ' - ' . $host)) . '</title>';
        }
        return '<title>' . ($title == '' ? $host : ($nohost ? $title. ' | '.env('APP_NAME') : ($host . ' - ' . $title))) . '</title>';
    }

    public static function debug($array, $print_r = false, $exit = false, $trace = false, $go = false){
        if(env('APP_DEBUG') || $go){
    ...
    public static function getLanguageKey(){
        return 'pam'.self::module_config('slug_name').'_lang';
    }

    public static function getDefaultLang(){
        if(empty(self::$defLang)){
            self::$defLang = Cookie::get(self::getLanguageKey(), env('APP_LANG'));
        }
        return self::$defLang;
    }

    public static function updateLang($lang){
        $opts = config('app.locales');
        if(!empty($lang) && isset($opts[$lang])) {
            Cookie::queue(self::getLanguageKey(), $lang, 60 * 24 * 365);
            \App::setLocale($lang);
        }
    }
    ...
    public static function is_mobile($value = '') {
        return preg_match('#^(01([0-9]{2})|09[0-9]|08[0-9]|07[0-9]|03[0-9])(\d{7})$#', $value);
    }
    ...
    public static function getFirstDayOfMonth($time = 0, $next = false){
        $temp = date('01/m/Y', $time);
        if($next){
            $temp = date("Y-m-01", $time);
            return strtotime('+1 month', strtotime($temp));
        }
        return \Lib::getTimestampFromVNDate($temp);
    }
    ...
    public static function str_limit_2($str, $limit = 60, $strip_tags = true){
        if($strip_tags) {
            $str = trim(strip_tags($str));
        }
        if( mb_strlen($str, "utf-8") > $limit ) {
            $surfix = "...";
            $strTmp = mb_substr($str, 0, $limit, "utf-8");
            $str = mb_substr($strTmp, 0, mb_strlen($str, "utf-8"), 'utf-8').$surfix;
        }
        return $str;
    }
    ...
    public static function addMedia($src = ''){
        $html = '';
        if(!empty($src)){
            $version = self::getSiteConfig('version', 1);
    ...
    static function callMobile($tel = 0, $nolink = false){
        $strip = preg_replace('/[^0-9]/', '', $tel);
        return $nolink ? $strip : '<a href="tel:'.$strip.'" alt="tel:'.$strip.'" title="tel:'.$strip.'">'.$tel.'</a>';
    }
